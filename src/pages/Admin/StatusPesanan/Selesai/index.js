...
const StatusSelesai = ({ navigation, route }) => {
  ...
  const processPressed = async () => {
    ...
    await fetch(`${API}/api/v1/owner/laundries/${data.laundry.id}/transaction/${data.id}`, {
      ...
      body: JSON.stringify({
        ...
        status: data.service_type == '1' ? STATUS_READY : STATUS_DELIVER
      })
    })
      ...
      <Animated.View
        ...
        <ScrollView style={{ padding: 20 }}>
          ...
          <View
            ...
            <Text style={globalStyles.bodyText}>{data.created_at}</Text>
          </View>
          <View
            style={{
              flexDirection: 'row',
            ...
            <Text style={{...globalStyles.bodyText }}>Tanggal Selesai </Text>
            <Text style={globalStyles.bodyText}>{data.updated_at}</Text>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
